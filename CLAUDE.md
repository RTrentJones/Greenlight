...
## Status: Phase 0 built (skeleton + seam); loop not yet built

The monorepo skeleton, manifest schema/loader, package boundaries, and the CI seam/boundary checks exist and pass. The deploy→validate→iterate loop (Phase 1+) does not exist yet. See [docs/phase-0-plan.md](docs/phase-0-plan.md) for what Phase 0 delivered and [docs/development.md](docs/development.md) for how to work in the repo.
...
- `pnpm greenlight agent sync` — materialize the agentic dev loop kit (deploy-verify-promote skill + `.mcp.json` + CLAUDE.md block) into a repo. The kit (skills + MCP servers + best practices) is [docs/agentic-loop.md](docs/agentic-loop.md); cross-repo via the Claude Code plugin: `/plugin marketplace add RTrentJones/greenlight`.
- `pnpm greenlight adopt <name> --repo <path> --lane <l> --target <t> [--data --auth --envs]` — poly-repo onboarding: scaffold a separate existing tool repo into a thin Greenlight consumer (manifest, merged `package.json` + vendored tarballs, infra, namespaced workflows, verify spec, agent kit) leaving app code untouched, and register it as an `external` pointer in the cwd manifest (the central registry). Run from your site repo. See greenlight-v1.md §8.
...
- **Verification is wired to promotion, not just test-writing.** `packages/verify` exposes one `verify(baseUrl, spec) -> {pass, report}` harness with modes by lane (`api`, `playwright` accessibility-tree, `mcp`). CI *and* the agent loop call the same harness. The loop: change → preview → `verify` → develop/beta → `verify` → `promote` (gated fast-forward develop→main) → prod → `verify`.
